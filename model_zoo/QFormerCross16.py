...
class SDPAMultiHeadAttention(nn.Module):
    ...
    def __init__(self, token_dim, num_heads, qk_norm=False):
        ...
        self.q_norm = (
            nn.RMSNorm(self.head_dim) if qk_norm else nn.Identity()
        )
        self.k_norm = (
            nn.RMSNorm(self.head_dim) if qk_norm else nn.Identity()
        )
...
class QFormerCross16(MultiTaskModel):
    """QFormerCross8 recurrence with RMS-normalized attention Q/K.
    ...
    stack. Attention Q/K heads use RMSNorm instead of LayerNorm. This file
    contains the complete model implementation and does not
    depend on another QFormerCross variant.
    """

    def __init__(self,
                 feature_map,
                 model_id="QFormerCross16",
        ...
        if not self.target_item_features:
            raise ValueError("QFormerCross16 requires target item features")
        if not self.sequence_features:
            raise ValueError(
                "QFormerCross16 requires item/action sequence features"
            )
        if not self.context_features:
            raise ValueError("QFormerCross16 requires user/context features")
