...
Traditional ranking pipelines commonly retain only positive behaviors in a user's history and supervise the newest impression. UniRank instead organizes impressions or interaction anchors chronologically. Each eligible step becomes a point-wise training sample, while earlier item/action/timestamp events form its causal history.
...
**Figure 1. From newest-impression supervision to chronological point-wise supervision.** The same user sequence can provide supervision at multiple chronological positions. Action-aware history tokens retain the type or multi-feedback pattern of previous events, while the current target is evaluated using its task labels.

For a target at position `t`, the dataloader uses only events before `t`, truncates or pads the history to `max_len`, and keeps the target item separate from its historical context. This increases sample utilization and lets models learn how preferences evolve through the sequence without inserting future tokens into the target history.
...
UniRank uses explicit train, validation, and test paths. The provided preprocessors generally create chronological or date-based splits: earlier samples train the model, a later interval selects the checkpoint, and the final interval is evaluated once. The same user may therefore appear in multiple splits, but each target only consumes its preceding history.

<p align="center">
  <img width="1050" alt="User-disjoint split versus chronological per-user split" src="./assets/figures/test_pipeline.png">
</p>

**Figure 2. User-disjoint and chronological evaluation protocols.** A user-level split measures cold-start generalization to unseen users. The chronological protocol used by the current benchmark instead measures warm-start ranking on future interactions of users observed in earlier periods, which more closely resembles recurring-user online ranking. It does not replace cold-start evaluation; the two protocols answer different questions.
