...
def print_stats(stats: dict):
    ...
    print("=" * 72)
    print(f"user 数量: {stats['user_count']:,}")
    print(f"item 数量: {stats['item_count']:,}")
    print(
        "样本数量: "
        f"{sample['total']:,} "
        f"(train={sample['train']:,}, valid={sample['valid']:,}, test={sample['test']:,})"
    )
    print(f"平均序列长度: {stats['avg_sequence_length']:.4f}")
    print(f"最大序列长度: {stats['max_sequence_length']:,}")
    print(f"feature field 数量: {stats['feature_field_count']:,}")
    print(f"任务数量: {stats['task_count']:,}")
    print("=" * 72)


def run_cli(spec: DatasetSpec):
    parser = argparse.ArgumentParser(description=f"统计 {spec.name} blocked 数据集信息")
    parser.add_argument(
        "--output_dir",
        type=str,
