#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""统计 Taobao_Action blocked 数据集信息。

除通用 blocked 数据集统计外，额外统计：
  - 每个 label 在 train/valid/test/total 中的正负样本数、正负样本率与正样本占比
  - action token 分布
  - meta_data.json 中的 behavior_log 匹配统计
"""
...
def compute_action_stats(output_dir: Path, action_vocab: dict, label_columns):
    ...
    Taobao 的 data parquet 不直接保存 `action` 列，`action` 只用于
    user_info.full_action_seq 和 dataloader。因此这里按 data 中的多任务
    label 列重建与预处理脚本一致的 action pattern，再映射到 meta_data.json
    中的 action_vocab。
    """
...
def run_cli():
    parser = argparse.ArgumentParser(description="统计 Taobao_Action blocked 数据集信息")
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(SPEC.default_output_dir),
        help="预处理脚本生成的 blocked 输出目录",
    )
    parser.add_argument(
        "--json",
        type=str,
